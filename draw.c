...
void add_circle( struct matrix * points,
		 ...
		 double r, double step ) {
			 ...
			 for (t = 0.0; t<(2*M_PI + .0000001); t+=step){
				 double x = r*cos(t) + cx;
				 double y = r*sin(t) + cy;
				 double x1 = r*cos(t+step) + cx;
				 double y1 = r*sin(t+step) + cy;
...
void add_curve( struct matrix *points,
		...
		double step, int type ) {
			...
			for (t = step; t<1+step; t+=step){
				...
				double pty1 = (y->m[0][0] * pow(t, 3)) + (y->m[1][0] * pow(t, 2)) + (y->m[2][0] * t) + (y->m[3][0]);

				add_edge(points,ptx0,pty0,0,ptx1,pty1,0);
